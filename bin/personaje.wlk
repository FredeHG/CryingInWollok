...
class Personaje {
	...
	method mejorArtefacto() = artefactos.filter({artefacto => !artefacto.esEspejoFantastico()}).max({artefacto => artefacto.unidadesDeLucha(self)})
	...
	method comprarArtefacto(artefacto,comerciante){
		if(!self.puedeCostear(comerciante.precio(artefacto))) 
			self.error("Error al realizar la compra: el personaje no tiene suficiente dinero")
		if(!self.puedeLlevar(artefacto))
			self.error("Error al realizar la compra: el personaje no tiene suficiente espacio")
